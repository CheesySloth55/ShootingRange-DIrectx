...
bool AlphaMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	// shader files are located in engine/data/shaderfiles/XXXX
	error = wcscpy_s(vsFileName, 128, L"engine/data/shaderfiles/alphamapvs.hlsl");
	if (error != 0)
	{
		return false;
	}
	error = wcscpy_s(psFileName, 128, L"engine/data/shaderfiles/alphamapps.hlsl");
	if (error != 0)
	{
		return false;
	}
