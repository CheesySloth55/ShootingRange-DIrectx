...
bool SpecMapShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
    ...
    error = wcscpy_s(vsFilename, 128, L"engine/data/shaderfiles/specmapvs.hlsl");
    if (error != 0)
    {
        return false;
    }
    ...
    error = wcscpy_s(psFilename, 128, L"engine/data/shaderfiles/specmapps.hlsl");
    if (error != 0)
    {
        return false;
    }
...
{
    using namespace Microsoft::WRL; // fpr com ptr;
    ...
    //setup dynamic matrix constant buffer for the vertexShader
    ...
    XMFLOAT3 lightDirection, XMFLOAT4 diffuseColor, XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower)
{
    ...
    //set textures to input registers
